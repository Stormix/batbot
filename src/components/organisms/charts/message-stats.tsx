...
const MessageStats = ({ className }: { className?: string }) => {
  const [activeChart, setActiveChart] = React.useState<keyof typeof chartConfig>('messages');
  const total = React.useMemo(() => chartData.reduce((acc, curr) => acc + curr.messages, 0), []);
    ...
    <Card className={className}>
      <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-0 sm:flex-row">
        <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
          ...
          <CardDescription>Showing total chat messages for the last 3 months</CardDescription>
        </div>
        <div className="flex">
          <button
            data-active
